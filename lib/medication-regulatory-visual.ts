...
export type MedicationRegulatoryTone =
  | "neutral"
  | "red"
  | "black"
  | "yellow"
...
export type MedicationRegulatorySurface = {
  accent: string;
  rail: string;
  badgeClass: string;
  iconClass: string;
...
const UNKNOWN: MedicationRegulatoryVisual = {
  ...
  detail:
    "O Vault ainda não confirmou uma classificação regulatória para este medicamento.",
  verified: false,
  accent: "#64748b",
  badgeClass:
...
function visualForModel(
  code?: MedicationRegulatoryPrescriptionModelCode,
  model?: string
): MedicationRegulatoryVisual | null {
  ...
  switch (code) {
    case "notificacao_b":
      return {
        tone: "black",
        label: "Notificação B",
        detail: model || "Receituário de controle especial",
        verified: true,
        sourceLabel: source,
        accent: "#94a3b8",
        badgeClass:
          "border-slate-300/35 bg-black/70 text-slate-100",
      };

    case "notificacao_b2":
      return {
        tone: "black",
        label: "Notificação B2",
        detail: model || "Receituário de controle especial",
        verified: true,
        sourceLabel: source,
        accent: "#94a3b8",
    ...
    case "notificacao_retinoides":
      return {
        ...
        detail: model || "Retinoides de uso sistêmico",
        verified: true,
        sourceLabel: source,
        accent: "#fb7185",
        badgeClass:
    ...
    case "receita_controle_especial":
      return {
        ...
        detail: model || "Receita de controle especial",
        verified: true,
        sourceLabel: source,
        accent: "#f87171",
        badgeClass:
    ...
    case "receita_comum":
      return {
        tone: "neutral",
        label: "Receita comum",
        detail: model || "Venda sob prescrição",
        verified: true,
        sourceLabel: source,
        accent: "#cbd5e1",
        badgeClass:
...
function visualForLegacy(
  ...
  > = {
    ...
    azul: {
      ...
      accent: "#94a3b8",
      badgeClass:
        "border-slate-300/30 bg-black/70 text-slate-100",
    },
    branca: {
      tone: "red",
      label: "Controle especial",
...
export function resolveMedicationRegulatoryVisual(
  medication: Medicamento,
  reference?: MedicationReference | null,
  at = new Date()
): MedicationRegulatoryVisual {
  const rules = (
    reference?.regulatoryRules || []
  ).filter((rule) =>
    isMedicationRegulatoryRuleActive(rule, at)
  );
  ...
  if (rules.length) {
    ...
    const visuals = safeRules
      .map((rule) =>
        visualForModel(
          rule.prescriptionModelCode,
      ...
      .filter(
        (
          item
        ): item is MedicationRegulatoryVisual =>
          Boolean(item)
      );
...
export function getMedicationRegulatorySurface(
  profile?: MedicationRegulatoryVisual | null,
  ...
  if (tone === "black") {
    return {
      ...
      rail: "#94a3b8",
      badgeClass:
        "border-slate-300/35 bg-black/70 text-slate-100 ring-1 ring-inset ring-white/10",
      iconClass:
        "border-slate-300/30 bg-black/65 text-slate-100 ring-1 ring-inset ring-white/10",
      dotClass:
        "border-slate-200/45",
      glowClass:
  ...
  if (tone === "neutral") {
    return {
      accent:
        profile?.accent || "#e2e8f0",
      rail: "#cbd5e1",
      badgeClass:
        profile?.badgeClass ||
        "border-slate-300/25 bg-slate-300/10 text-slate-200",
      iconClass:
        "border-slate-200/25 bg-slate-100/10 text-slate-100",
      dotClass:
        "border-slate-200/30",
      glowClass: "",
    };
