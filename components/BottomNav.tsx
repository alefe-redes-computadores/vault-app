...
const SAUDE_COMPOSE_OPTIONS: ComposeOption[] = [
  {
    ...
    label: "Medicamento",
    icon: Pill,
    path: "/saude/medicamentos/novo",
  },
  {
    id: "tratamento",
    label: "Tratamento",
    icon: FolderHeart,
    path: "/saude/tratamentos/novo",
  },
  {
    id: "renovacao",
    label: "Renovação",
    icon: FileWarning,
    path: "/saude/renovacao/nova",
  },
  {
    id: "medico",
    label: "Médico",
    icon: Stethoscope,
    path: "/saude/medicos/novo",
  },
  {
    id: "farmacia",
    label: "Farmácia",
  ...
  },
  {
    ...
    label: "Posto / Local",
    icon: MapPin,
    path: "/saude/locais/novo",
  },
  {
    id: "exame",
    label: "Exame",
    icon: FlaskConical,
    path: "/saude/exames/novo",
  },
  {
    id: "consulta",
    label: "Consulta",
    icon: Calendar,
    path: "/saude/consultas/nova",
  },
  {
    id: "cirurgia",
    label: "Cirurgia",
    icon: Syringe,
    path: "/saude/cirurgias/nova",
  },
  {
    id: "cid",
    label: "CID",
    icon: FileText,
    path: "/saude/cids/novo",
  },
  {
    id: "sintoma",
    label: "Sintoma / Medição",
    icon: Activity,
...
const SAUDE_DOCUMENTOS_COMPOSE_OPTIONS: ComposeOption[] = [
  {
    id: "novo-documento-saude",
    label: "Novo documento",
    icon: Plus,
    path: "/saude/documentos/novo",
  },
];
...
function getOptionColorClass(
  id: string
) {
  if (["dose-unica"].includes(id)) {
    return "bg-emerald-400/10 text-emerald-400 border-emerald-400/20";
  }

  if (["sintoma"].includes(id)) {
    return "bg-amber-400/10 text-amber-400 border-amber-400/20";
  }

  if (["renovacao"].includes(id)) {
    return "bg-coral/10 text-coral border-coral/20";
  }

  if (
    ["medicamento", "farmacia"].includes(
      id
    )
  ) {
    return "bg-amber-400/10 text-amber-400 border-amber-400/20";
...
export function BottomNav() {
  ...
  const handleComposeOptionPress =
    (
          ...
          showCompose && (
            <>
              ...
              <motion.div
                ...
                className="fixed bottom-[6.5rem] left-4 right-4 z-50 mx-auto max-w-sm overflow-hidden rounded-[24px] border border-surface-border/80 bg-surface/95 shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-xl"
              >
                ...
                <div className="scrollbar-hide max-h-[60vh] overflow-y-auto px-3 pb-4 pt-1">
                      ...
                      (
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              handleComposeOptionPress(
                                option
                              )
                            }
                            className="flex items-center gap-2.5 rounded-xl border border-surface-border/40 bg-surface-raised/40 p-2 text-left transition-all hover:bg-surface-raised active:scale-95"
                          >
                            <div
                            ...
                            <span className="line-clamp-2 text-[11px] font-medium leading-tight text-ink-primary">
                              {
                                option.label
                              }
                            </span>
