...
export function HealthNotifications() {
  ...
  const { alertas } = useMemo(() => {
    ...
    // Ordenar: Pendentes (atrasadas) primeiro, Hoje em segundo, Próximas em terceiro
    itensAlerta.sort((a, b) => {
      const peso = { pendente: 1, hoje: 2, proxima: 3 };
      if (peso[a.urgencia as keyof typeof peso] !== peso[b.urgencia as keyof typeof peso]) {
        return peso[a.urgencia as keyof typeof peso] - peso[b.urgencia as keyof typeof peso];
      }
  ...
  const getUrgencyColor = (urgencia: string) => {
    switch (urgencia) {
      case "pendente": return "bg-coral/10 border-coral/30 text-coral";
      case "hoje": return "bg-amber-400/10 border-amber-400/30 text-amber-400";
      case "proxima": return "bg-ice/10 border-ice/30 text-ice";
      default: return "bg-surface-raised border-surface-border text-ink-primary";
    }
  };
  ...
  return (
    <div className="w-full space-y-3 mb-6">
      <div className="flex items-center gap-2 px-1">
        ...
        <h3 className="font-display text-sm font-semibold text-ink-primary">Alertas de Saúde</h3>
      </div>
        ...
        <AnimatePresence>
          {alertas.slice(0, 3).map((alerta) => (
            <motion.div
              ...
              onClick={() => {
                trigger("vibrate");
                router.push(`/saude/${alerta.tipo === "consulta" ? "consultas" : "cirurgias"}/detalhes?id=${alerta.id}`);
              }}
            ...
            >
              <div className="flex items-center gap-3.5 min-w-0">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${
                  alerta.urgencia === "pendente" ? "bg-coral/20 border-coral/30" : 
                  alerta.urgencia === "hoje" ? "bg-amber-400/20 border-amber-400/30" : 
                  "bg-ice/20 border-ice/30"
                }`}>
                  {alerta.urgencia === "pendente" ? <AlertCircle size={18} /> : 
                   alerta.urgencia === "hoje" ? <CalendarClock size={18} /> : 
                   alerta.tipo === "consulta" ? <Stethoscope size={18} /> : <Activity size={18} />}
                </div>
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate font-medium text-sm mt-0.5">
                    {alerta.tipo === "consulta" ? alerta.motivo || "Consulta Agendada" : alerta.procedimento}
                  </p>
                </div>
              </div>
