        ...
        <section className="space-y-5 px-5 pt-6">
          <div className="rounded-[28px] border border-surface-border/50 bg-surface px-5 py-6 shadow-sm">
            <div className="mb-6 flex flex-col items-center text-center">
              <div className="flex h-24 w-24 items-center justify-center rounded-[28px] border border-white/5 shadow-sm" style={{ backgroundColor: `${formData.color}1F` }}>
                <SelectedIcon size={34} style={{ color: formData.color }} />
              </div>
              <h2 className="mt-4 font-display text-lg font-semibold text-ink-primary">Configure seu cofre</h2>
              <p className="mt-2 max-w-xs text-sm leading-6 text-ink-muted">Escolha um nome, uma cor e um ícone para identificar rapidamente este espaço.</p>
            </div>
          ...
          <div className="rounded-[28px] border border-surface-border/50 bg-surface px-5 py-5 shadow-sm">
            <div className="mb-3">
              ...
              <p className="mt-1 text-xs text-ink-muted">Escolha o símbolo que melhor representa este cofre.</p>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-[74px] flex-col items-center justify-center gap-1.5 rounded-2xl border px-2 py-3 transition-all duration-200 active:scale-95 ${
                      isSelected ? "border-ice bg-ice/10 text-ice shadow-[0_0_0_1px_rgba(125,211,252,0.08)]" : "border-surface-border/50 bg-surface-raised text-ink-muted hover:text-ink-primary"
                    }`}
                  >
                    <Icon size={20} />
          ...
          <div className="rounded-[28px] border border-surface-border/50 bg-surface px-5 py-5 shadow-sm">
            <div className="mb-3">
              ...
              <p className="mt-1 text-xs text-ink-muted">Use uma cor para diferenciar cofres com mais rapidez.</p>
            </div>
              ...
              {COLOR_OPTIONS.map((color) => {
                ...
                return (
                  <button
                    ...
                    className={`relative h-10 w-10 rounded-full border-2 transition-all duration-200 active:scale-95 ${selected ? "border-white shadow-lg" : "border-transparent"}`}
                    style={{ backgroundColor: color }}
                  >
                    {selected && <span className="absolute inset-0 rounded-full ring-4 ring-white/10" />}
                  </button>
                );
              })}
            </div>
          ...
          <div className="rounded-[28px] border border-surface-border/50 bg-surface px-5 py-5 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-ink-muted">Prévia</p>
            <div className="mt-4 flex items-center gap-4 rounded-[24px] border border-surface-border/50 bg-surface-raised/60 px-4 py-4">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl" style={{ backgroundColor: `${formData.color}22` }}>
                <SelectedIcon size={22} style={{ color: formData.color }} />
              </div>
              <div className="min-w-0">
                <p className="truncate font-display text-base font-semibold text-ink-primary">{formData.name || "Nome do cofre"}</p>
                <p className="mt-1 line-clamp-2 text-xs leading-5 text-ink-muted">{formData.description || "Descrição opcional do cofre"}</p>
              </div>
            </div>
          </div>
          ...
          >
            {loading ? <><Loader2 size={16} className="animate-spin" /> Criando...</> : "Criar cofre"}
          </Button>
        </section>
      </main>
    </PageTransition>
