...
import { ArrowLeft, Plus, Store, Trash2, Edit, Loader2 } from "lucide-react";
        ...
        <header className="sticky top-0 z-10 bg-void/80 backdrop-blur-xl border-b border-surface-border/30 px-5 pt-6 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display text-xl font-semibold text-ink-primary">Farmácias</h1>
                <p className="text-sm text-ink-muted">
                  {farmacias.length} farmácia{farmacias.length !== 1 ? "s" : ""} cadastrada{farmacias.length !== 1 ? "s" : ""}
                </p>
              </div>
            </div>
            <button
        ...
        <section className="px-5 pt-6 space-y-4">
          {farmacias.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="font-display text-lg text-ink-primary">Nenhuma farmácia</h3>
              <p className="text-sm text-ink-muted mt-1 max-w-xs">
                Cadastre farmácias para facilitar o preenchimento de receitas.
              </p>
              <button
                onClick={() => {
                  trigger("vibrate");
            ...
            farmacias.map((farmacia, index) => (
              ...
              >
                <div>
                  ...
                  {farmacia.endereco && (
                    <p className="text-sm text-ink-muted">{farmacia.endereco}</p>
                  )}
                  {farmacia.telefone && (
                    <span className="text-xs text-ink-muted">{farmacia.telefone}</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => handleDeleteClick(farmacia.id!, farmacia.nome)}
