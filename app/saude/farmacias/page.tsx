      ...
      <main className="min-h-screen bg-void pb-28">
        <header className="sticky top-0 z-20 border-b border-surface-border/30 bg-void/82 px-5 pb-4 header-safe-top backdrop-blur-xl">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            ...
            {filtroStatus !== "todos" && (
              <button onClick={() => { trigger("vibrate"); setFiltroStatus("todos"); }} className="text-[10px] font-medium text-coral bg-coral/10 px-2.5 py-1 rounded-full flex items-center gap-1">
                <X size={12} /> Limpar
              </button>
            )}
          </div>
        ...
        <section className="px-5 pt-5 space-y-3.5">
          {filteredFarmacias.length === 0 ? (
            <EmptyState icon={Store} title="Nenhuma farmácia encontrada" description={search || filtroStatus !== "todos" ? "Tente ajustar os filtros aplicados." : "Cadastre farmácias para acompanhar histórico de preços e renovações."} />
          ) : (
                  ...
                  {farmacia.ultimosMedicamentos.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {farmacia.ultimosMedicamentos.map((nome, idx) => (
                        <span key={idx} className="text-[9px] font-bold uppercase px-2 py-0.5 rounded-full bg-surface-raised border border-surface-border/40 text-ink-muted">{nome}</span>
                      ))}
                    </div>
                  )}
