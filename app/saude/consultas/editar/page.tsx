...
function EditarConsultaContent() {
  ...
  useEffect(() => {
    ...
    const loadConsulta = async () => {
      const data = await db.consultas.get(id);
      if (data) {
    ...
    await run(
      async () => {
        ...
        await db.transaction("rw", db.consultas, db.syncQueue, async () => {
            const original = await db.consultas.get(id);
            ...
            await db.consultas.put(consultaAtualizada);
            await enfileirarOperacao("consultas", "update", consultaAtualizada);
        });
      },
      {
