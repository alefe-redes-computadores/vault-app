...
import { 
  ArrowLeft, Loader2, Save, Pill, Trash2, AlertTriangle, Package, Plus, Clock, Activity, Brain, ShieldAlert, HeartPulse, Flame, FileWarning, ExternalLink, History
} from "lucide-react";
...
export default function EditarMedicamentoPage() {
  ...
  const diasEstimados = estoqueAtivo && consumoDiario > 0 && Number(estoqueQuantidade) > 0 ? Math.floor(Number(estoqueQuantidade) / consumoDiario) : null;
    ...
    getMedicamento(id).then(async (item) => {
      if (!item) {
        ...
        if (item.document_id) {
          ...
          if (doc && doc.metadata?.tratamento_id) setTratamentoId(doc.metadata.tratamento_id);
        }

        if (typeof item.estoque_quantidade === "number" && item.estoque_data_referencia && item.estoque_horarios && item.estoque_horarios.length > 0) {
  ...
  const handleCreateTratamento = async () => {
    ...
    trigger("vibrate");
    try {
      const newId = await safeAddTratamento({ user_id: user?.id || "", nome: newTratamentoName.trim(), status: "ativo" });
  ...
  const validate = (): boolean => {
    ...
    if (estoqueAtivo) {
      if (!estoqueQuantidade || Number(estoqueQuantidade) <= 0) newErrors.estoqueQuantidade = "Informe a quantidade atual";
      if (!estoqueDataReferencia) newErrors.estoqueDataReferencia = "Informe a data dessa contagem";
      const horariosPreenchidos = horarios.filter((h) => h);
      if (horariosPreenchidos.length === 0) newErrors.horarios = "Adicione pelo menos um horário";
    }
      ...
      if (documentId) {
        ...
        if (doc && doc.id) {
          await db.documents.update(doc.id, {
            metadata: { ...doc.metadata, tratamento_id: tratamentoId || undefined },
    ...
    return (
      <PageTransition>
        <main className="flex min-h-screen flex-col items-center justify-center bg-void px-6 text-center">
          <p className="font-display text-lg font-semibold text-ink-primary">Medicamento não encontrado</p>
    ...
    <PageTransition>
      <main className="min-h-screen bg-void pb-[calc(8rem+env(safe-area-inset-bottom))]">
        <header className="sticky top-0 z-20 border-b border-surface-border/30 bg-void/82 px-5 pb-4 header-safe-top backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="mt-1 font-display text-xl font-semibold text-ink-primary">Detalhes do Medicamento</h1>
            </div>
        ...
        <section className="space-y-4 px-5 pt-6">
          ...
          <motion.div variants={fadeUp} initial="initial" animate="animate" className="rounded-[28px] border border-surface-border/50 bg-surface p-4 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-ink-muted mt-0.5">
                {statusAtivo ? "Remédio em uso contínuo." : "Medicamento descontinuado ou substituído."}
              </p>
            </div>
            <button
          ...
          <motion.div variants={fadeUp} initial="initial" animate="animate" transition={{ delay: 0.01 }} className="rounded-[28px] border border-violet-500/30 bg-surface p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              <Activity size={16} className="text-violet-400" />
              <label className="text-sm font-semibold text-ink-primary">Tratamento Vinculado</label>
            </div>
            <button onClick={() => { trigger("vibrate"); setIsTratamentoModalOpen(true); }} className="flex w-full items-center justify-between rounded-2xl border border-surface-border/50 bg-surface-raised px-4 py-3.5 text-left text-ink-primary transition-colors hover:border-violet-400/40">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="truncate font-medium">{selectedTratamento ? selectedTratamento.nome : "Vincular a um tratamento"}</span>
              </div>
          ...
          <motion.div variants={fadeUp} initial="initial" animate="animate" transition={{ delay: 0.02 }} className="rounded-[28px] border border-surface-border/50 bg-surface p-4 shadow-sm">
            <p className="mb-3 text-sm font-medium text-ink-primary">Cor da receita</p>
          ...
          <motion.div variants={fadeUp} initial="initial" animate="animate" transition={{ delay: 0.04 }} className="space-y-3 rounded-[28px] border border-surface-border/50 bg-surface p-4 shadow-sm">
            <Input label="Medicamento" placeholder="Ex: Metadona, Sertralina..." value={nome} onChange={(e) => setNome(e.target.value)} error={errors.nome} required />
            <Input label="Dosagem" placeholder="Ex: 10mg, 1-0-1" value={dosagem} onChange={(e) => setDosagem(e.target.value)} error={errors.dosagem} required />
            
            <div>
              <label className="mb-1.5 block text-sm font-medium text-ink-primary">Médico <span className="text-coral">*</span></label>
              <button onClick={() => { trigger("vibrate"); setIsDoctorModalOpen(true); }} className={`w-full rounded-2xl border px-4 py-3 text-left text-ink-primary transition-colors ${errors.medico ? "border-coral/50" : "border-surface-border/50"} bg-surface-raised`}>
                {medicoNome || "Selecionar médico"}
              </button>
            </div>
            
            <div>
              <label className="mb-1.5 block text-sm font-medium text-ink-primary">Farmácia / Hospital (opcional)</label>
              <button onClick={() => { trigger("vibrate"); setIsPharmacyModalOpen(true); }} className="w-full rounded-2xl border border-surface-border/50 bg-surface-raised px-4 py-3 text-left text-ink-primary transition-colors">
                {farmaciaNome || "Selecionar local"}
              </button>
            </div>
          </motion.div>
            ...
            <button onClick={toggleEstoque} className="flex w-full items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-left">
                  <p className="text-sm font-medium text-ink-primary">Acompanhar estoque / Alarme</p>
                </div>
              </div>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                  <div className="mt-4 space-y-3 border-t border-surface-border/40 pt-4">
                    <div className="flex items-center justify-between gap-2 rounded-2xl bg-surface-raised/60 px-3 py-2.5">
                      <p className="text-xs text-ink-muted">Contagem base: <span className="font-medium text-ink-primary">{estoqueDataReferencia}</span></p>
                      <button onClick={registrarContagemHoje} className="shrink-0 text-xs font-medium text-ice hover:text-ice/80">Recontar hoje</button>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <Input label="Quantidade" type="number" value={estoqueQuantidade} onChange={(e) => setEstoqueQuantidade(e.target.value)} error={errors.estoqueQuantidade} />
                      <Input label="Unidade" value={estoqueUnidade} onChange={(e) => setEstoqueUnidade(e.target.value)} />
                    </div>
                    <Input label="Unid. por dose" type="number" value={estoqueUnidadePorDose} onChange={(e) => setEstoqueUnidadePorDose(e.target.value)} />
                    <div>
                        ...
                        {horarios.map((horario, index) => (
                          <div key={index} className="flex items-center gap-2">
                            <div className="relative flex-1">
                              ...
                              <input type="time" value={horario} onChange={(e) => updateHorario(index, e.target.value)} className="w-full rounded-2xl border border-surface-border/50 bg-surface-raised py-3 pl-9 pr-3 text-ink-primary outline-none transition-all focus:border-ice/50" />
                            </div>
                            {horarios.length > 1 && (
                              <button onClick={() => removeHorario(index)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-ink-muted transition-colors hover:bg-surface-border/40 hover:text-coral"><Trash2 size={14} /></button>
                            )}
                          </div>
                        ))}
                      </div>
                    ...
                    {diasEstimados !== null && (
                      <div className="rounded-2xl bg-ice/8 px-3 py-2.5 text-xs text-ink-muted">
                        Dura <span className="font-medium text-ink-primary">{diasEstimados} dia{diasEstimados !== 1 ? "s" : ""}</span> a partir da contagem.
                      </div>
                    )}
                  </div>
                </motion.div>
          ...
          <motion.div variants={fadeUp} initial="initial" animate="animate" transition={{ duration: 0.28, delay: 0.08 }} className="rounded-[28px] border border-surface-border/50 bg-surface p-4 shadow-sm">
            <TextArea label="Notas gerais do tratamento (Opcional)" placeholder="Ex: Tomar em jejum..." value={observacoes} onChange={(e) => setObservacoes(e.target.value)} />
          </motion.div>
        </section>
        ...
        <BottomSheet isOpen={isCreatingTratamento} onClose={() => { trigger("vibrate"); setIsCreatingTratamento(false); setNewTratamentoName(""); }} title="Cadastrar Tratamento"> <div className="space-y-4 px-1 pb-2"> <Input label="Nome do Tratamento" placeholder="Ex: Dor Crônica, Ansiedade..." value={newTratamentoName} onChange={(e) => setNewTratamentoName(e.target.value)} autoFocus /> <Button variant="primary" fullWidth onClick={handleCreateTratamento} disabled={isSavingTratamento || !newTratamentoName.trim()} className="flex items-center justify-center gap-2"> {isSavingTratamento ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />} Salvar e selecionar </Button> </div> </BottomSheet>
        <SelectionModal isOpen={isDoctorModalOpen} onClose={() => setIsDoctorModalOpen(false)} onSelect={(item: any) => { trigger("vibrate"); setMedicoNome(item.nome); }} items={medicos} title="Selecionar médico" placeholder="Buscar médico..." renderItem={(item: any) => ( <div><p className="font-medium text-ink-primary">{item.nome}</p>{item.especialidade && <p className="text-xs text-ink-muted">{item.especialidade}</p>}</div> )} getItemId={(item: any) => item.id!} getItemLabel={(item: any) => item.nome} onCreateNew={() => { setIsDoctorModalOpen(false); trigger("vibrate"); router.push("/saude/medicos/novo"); }} createNewLabel="Criar médico" />
        <SelectionModal isOpen={isPharmacyModalOpen} onClose={() => setIsPharmacyModalOpen(false)} onSelect={(item: any) => { trigger("vibrate"); setFarmaciaNome(item.nome); }} items={farmacias} title="Selecionar local" placeholder="Buscar farmácia ou hospital..." renderItem={(item: any) => ( <div><p className="font-medium text-ink-primary">{item.nome}</p>{item.endereco && <p className="text-xs text-ink-muted">{item.endereco}</p>}</div> )} getItemId={(item: any) => item.id!} getItemLabel={(item: any) => item.nome} onCreateNew={() => { setIsPharmacyModalOpen(false); trigger("vibrate"); router.push("/saude/farmacias/novo"); }} createNewLabel="Criar farmácia" />
        <ConfirmationModal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} onConfirm={handleDelete} title="Excluir medicamento" message={`Tem certeza que deseja excluir "${nome}"? Histórico de receitas não será apagado do banco de imagens.`} confirmLabel="Excluir" cancelLabel="Cancelar" isLoading={deleting} type="danger" />
      </main>
    </PageTransition>
  );
}
