    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (m) =>
          ...
          (m.medico && m.medico.toLowerCase().includes(q))
      );
    }
        ...
        <header className="sticky top-0 z-20 border-b border-surface-border/30 bg-void/82 px-5 pb-4 header-safe-top backdrop-blur-xl space-y-3 pt-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  trigger("vibrate");
                  setShowDescontinuados(!showDescontinuados);
                }}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                trigger("vibrate");
                setSortBy(e.target.value as "urgency" | "name" | "renewal");
              }}
            ...
            filteredAndSorted.map((med: Medicamento) => {
              ...
              return (
                <motion.button
                  ...
                  className={`w-full rounded-[24px] border bg-surface p-4 text-left shadow-md hover:bg-surface-raised relative overflow-hidden transition-all ${
                ...
                >
                  ...
                  <div className="flex items-start gap-4 ml-1">
                    <div
                      className="h-12 w-12 rounded-2xl flex items-center justify-center border border-surface-border shadow-inner shrink-0"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline gap-2 overflow-hidden flex-wrap">
                        <p className="font-display text-base font-bold text-ink-primary uppercase truncate">
                          {med.nome}
                        </p>
                        ...
                        {isSuspenso && (
                          <span className="shrink-0 rounded-full bg-coral/10 px-2 py-0.5 text-[9px] font-bold text-coral border border-coral/20 uppercase">
                            Suspenso
                          </span>
                        )}
                      </div>
