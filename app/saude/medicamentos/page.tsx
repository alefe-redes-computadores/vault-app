...
import { processarListaMedicamentos } from "@/lib/health-insights";
...
export default function MedicamentosListPage() {
  ...
  // Alerta de Doses Pendentes (CORRIGIDO PARA "info")
  useEffect(() => {
        ...
        <section className="space-y-3.5 px-5 pt-4">
          {listaProcessada.length === 0 ? (
            <EmptyState
              ...
              description={searchQuery.trim() ? "Não encontramos medicamentos para essa busca." : "Cadastre um medicamento para começar."}
              actionLabel={searchQuery.trim() || showDescontinuados ? "Limpar filtros" : undefined}
              onAction={searchQuery.trim() || showDescontinuados ? () => { setSearchQuery(""); setShowDescontinuados(false); trigger("vibrate"); } : undefined}
            />
          ) : (
            listaProcessada.map(({ med, isSOS, isSuspenso, foiTomadoHoje, horarioTomado, insight, receita, textoEstoque, isEstoqueZerado, isEstoqueCritico }) => {
              ...
              return (
                ...
                >
                  <div className="flex flex-col h-full min-h-[96px]">
                    ...
                    {/* --- LINHA 1: Nome, Dosagem e Alerta Minimizados --- */}
                    <div className="flex min-w-0 items-center justify-between gap-2">
                      ...
                      {/* CORRIGIDO PARA "info" ao invés de "warning" no Toast */}
                      {insight?.deveRenovar && (
                        <button
                          type="button"
                          onClick={(e) => { 
                            e.stopPropagation(); 
                            trigger("light");
                            showToast(insight.mensagem, insight.urgencia === "alta" ? "error" : "info");
                          }}
                    ...
                    {/* --- LINHA 2: Metadados Agrupados (Receita C1, Medico, Farmácia, Estoque) --- */}
                    <div className="mt-1 flex flex-wrap items-center gap-x-1.5 gap-y-1 text-[10px] text-ink-muted">
                      {receita && (
                        <button 
                          onClick={(e) => { e.stopPropagation(); trigger("light"); showToast(receita.tooltip, "info"); }}
                    ...
                    {/* --- LINHA 3: AÇÕES E ESTADOS (Empurradas para a base do card com mt-auto) --- */}
                    <div className="mt-auto pt-4 flex flex-wrap items-center justify-between gap-2">
                        ...
                        {!isSuspenso && (
                          foiTomadoHoje ? (
                            ...
                            !isEstoqueZerado && (
                              <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); trigger("vibrate"); setQuickDoseMedId(med.id!); }}
                                className="flex items-center gap-1 rounded-lg bg-emerald-500/10 px-2.5 py-1 text-[10px] font-bold text-emerald-400 active:scale-95 transition-transform"
                              >
                      ...
                      {!isSuspenso && insight.deveRenovar && (
                        <button
                          ...
                          className={`flex items-center gap-1 rounded-lg border px-2.5 py-1 text-[10px] font-bold transition-transform active:scale-95 ${insight.urgencia === "alta" ? "border-coral/30 bg-coral/10 text-coral" : "border-amber-400/30 bg-amber-400/10 text-amber-500"}`}
                        >
