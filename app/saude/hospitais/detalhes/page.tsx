...
function DetalhesHospitalContent() {
  ...
  const tratamentos = useLiveQuery(
    () =>
      tratamentoIds.length > 0
        ? db.tratamentos.where('id').anyOf(tratamentoIds).toArray()
  ...
  const analiseHospital = useMemo<AnaliseHospital>(() => {
    ...
    const cirurgiasDoHospital = [...cirurgias].sort(
      (a, b) => (b.data || "").localeCompare(a.data || "")
    );
    const renovacoesDoHospital = [...renovacoes].sort(
      (a, b) => (b.data || "").localeCompare(a.data || "")
    );
    const medicoIdsDiretos = hospital.medico_ids || [];
    const medicosDiretos = medicos.filter(
      (m) => m.id && medicoIdsDiretos.includes(m.id)
    );
    const medicoIdsInferidos = new Set(
      consultasDoHospital.map((c) => c.medico_id).filter(Boolean)
    );
    const medicosInferidos = medicos.filter(
      (m) => m.id && medicoIdsInferidos.has(m.id)
    );
    ...
    const ultimaConsulta =
      consultasDoHospital.length > 0 ? consultasDoHospital[0] : null;
            ...
            >
              ...
              <div className="mt-3 space-y-2">
                {alertasRelevantes.map((alerta, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-2 text-xs border-b border-amber-400/10 pb-2 last:border-0"
                  >
                    <AlertTriangle
                      size={14}
          ...
          >
                ...
                <div className="flex items-center gap-2 text-xs text-ink-muted">
                  ...
                  <span>
                    ...
                    <span className="font-medium text-ink-primary">
                      {formatDateDisplay(analiseHospital.ultimaConsulta.data)}
                    </span>
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3 pt-4 border-t border-surface-border/40">
              <StatCard
                ...
                label="Consultas"
                value={String(analiseHospital.consultas.length)}
              />
              <StatCard
                icon={<Activity size={14} />}
                label="Cirurgias"
                value={String(analiseHospital.cirurgias.length)}
              />
              <StatCard
                icon={<FileWarning size={14} />}
                label="Renovações"
                value={String(analiseHospital.renovacoes.length)}
              />
            </div>
          </motion.div>
              ...
              <div className="space-y-2">
                {analiseHospital.consultas.slice(0, 3).map((consulta) => (
                  <div
                    ...
                    onClick={() => {
                      trigger("vibrate");
                      router.push(`/saude/consultas/detalhes?id=${consulta.id}`);
                    }}
                ...
                {analiseHospital.consultas.length > 3 && (
                  <p className="text-[10px] text-center text-ink-muted pt-1">
                    E mais {analiseHospital.consultas.length - 3} registro(s)...
                  </p>
                )}
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-2">
                {analiseHospital.renovacoes.slice(0, 3).map((ren) => {
                  const vencida = isReceitaVencidaSegura(
                    ren.data_proxima_retirada || ren.data
                  );
                  return (
                    <div
                      key={ren.id}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-ink-muted">
                            {ren.observacoes || "Retirada de medicamento"}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <span className="text-xs font-semibold text-emerald-400">
                          {typeof ren.preco === "number" && ren.preco > 0
                            ? `R$ ${ren.preco.toFixed(2).replace(".", ",")}`
                            : "Gratuito"}
                        </span>
                      </div>
                    </div>
                ...
                {analiseHospital.totalGastoRenovacoes > 0 && (
                  <div className="mt-3 pt-3 border-t border-surface-border/40 flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-emerald-400">
                      R$ {analiseHospital.totalGastoRenovacoes.toFixed(2).replace(".", ",")}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-ink-muted">
                          {cir.data ? formatDateDisplay(cir.data) : "Data não informada"}
                        </p>
                      </div>
                    </div>
                    <ExternalLink size={15} className="text-ink-faint shrink-0" />
