...
import {
  ArrowLeft, Loader2, Save, Building2, Trash2, Activity, FlaskConical, Stethoscope, ExternalLink, Calendar,
} from "lucide-react";
...
function EditarHospitalContent() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      trigger("error");
      showToast("Erro ao atualizar hospital", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      trigger("success");
      showToast("Hospital excluído com sucesso", "success");
      router.replace("/saude/hospitais");
    } catch (error) {
      trigger("error");
      showToast("Erro ao excluir hospital", "error");
    } finally {
      setDeleting(false);
