...
import type { HealthTimelineEvent, PersonScoped } from "@/lib/health-insights";

import { formatCurrency, getClinicalTheme } from "@/lib/health-utils";
...
type DocumentPersonScoped = Document & {
  person_id?: string;
};
...
interface EconomiaMedicamento {
  ...
  economia: NonNullable<ReturnType<typeof calcularEconomia>>;
}
...
function formatDateDisplay(isoStr?: string | null): string {
  if (!isoStr) {
    return "";
  }
...
function getPrecoSeguro(value: Renovacao["preco"]): number {
...
>(item: T, personId: string): item is PersonScoped<T> {
  return item.person_id === personId;
}

function getTimelineHref(event: HealthTimelineEvent): string | null {
  if (!event.entidadeId) {
    return null;
  }

  const routes: Partial<Record<HealthTimelineEvent["tipo"], string>> = {
    renovacao: "/saude/renovacao/detalhes",

    registro_saude: "/saude/registros/detalhes",

    consulta: "/saude/consultas/detalhes",

    exame: "/saude/exames/detalhes",

    cirurgia: "/saude/cirurgias/detalhes",
  };

  const route = routes[event.tipo];

  return route ? `${route}?id=${encodeURIComponent(event.entidadeId)}` : null;
}

function TimelineEventIcon({ tipo }: { tipo: HealthTimelineEvent["tipo"] }) {
  const Icon =
    tipo === "consulta"
      ? Stethoscope
      : tipo === "exame"
...
function getStatusLabel(status: Tratamento["status"]): string {
  if (status === "ativo") {
    return "Em andamento";
  }
...
function TratamentoContent() {
  ...
  const { registros = [] } = useRegistrosSaude();

  const documentosDaPessoa = useDocuments();

  const { favoriteDocument } = useDocumentActions();
  ...
  const [dismissEconomia, setDismissEconomia] = useState(false);
  ...
  useEffect(() => {
    ...
    setDismissEconomia(stored === "true");
  }, [id]);
  ...
  const linkedRenovacoes = useMemo(() => {
    const medIds = new Set(
      linkedMedicamentos
        ...
        .filter((medicamentoId): medicamentoId is string =>
          Boolean(medicamentoId),
        ),
    );

    return renovacoes
      .filter((renovacao) => medIds.has(renovacao.medicamento_id))
      .sort((a, b) =>
        String(b.data_aquisicao || b.data || "").localeCompare(
          String(a.data_aquisicao || a.data || ""),
        ),
      );
  ...
  const linkedDocuments = useMemo(() => {
    ...
    return (documentosDaPessoa as DocumentPersonScoped[])
      .filter((document) => {
        if (document.person_id !== activePersonId) {
          return false;
        }
        ...
        return (
          metadata.tratamento_id === id ||
          (document.entidade_tipo === "tratamento" &&
            document.entidade_id === id)
        );
      })
      .sort((a, b) =>
        String(b.created_at || "").localeCompare(String(a.created_at || "")),
      );
  }, [documentosDaPessoa, activePersonId, id]);

  // ==========================================================
  // HISTÓRICO CLÍNICO RELACIONAL
  // ==========================================================

  const linkedConsultas = useMemo(
    () =>
      consultas.filter(
        (consulta) => consulta.tratamento_ids?.includes(id) === true,
      ),
    [consultas, id],
  );

  const linkedExames = useMemo(
    () => exames.filter((exame) => exame.tratamento_ids?.includes(id) === true),
    [exames, id],
  );

  const linkedCirurgias = useMemo(
    () =>
      cirurgias.filter(
        (cirurgia) => cirurgia.tratamento_ids?.includes(id) === true,
      ),
    [cirurgias, id],
  );

  const linkedRegistros = useMemo(
    () =>
      registros.filter(
        (registro) => registro.tratamento_ids?.includes(id) === true,
      ),
    [registros, id],
  );
  ...
  const cidsVinculados = useMemo(() => {
    const ids = new Set(tratamento?.cid_ids || []);

    if (ids.size === 0) {
      return [];
    }

    return cids.filter((cid) => Boolean(cid.id && ids.has(cid.id)));
  }, [cids, tratamento?.cid_ids]);

  const cidsInsights = useMemo(() => {
    return cidsVinculados.map((cid: Cid) => ({
      ...cid,

      insight: getCidInsights(cid.codigo),
    }));
  }, [cidsVinculados]);
  ...
  const linkedMedicos = useMemo(() => {
    ...
    for (const medicoId of tratamento?.medico_ids || []) {
      ids.add(medicoId);
    }

    for (const medicamento of linkedMedicamentos) {
      if (medicamento.medico_id) {
        ids.add(medicamento.medico_id);
      }
    }

    return medicos.filter((medico: Medico) =>
      Boolean(medico.id && ids.has(medico.id)),
    );
  }, [medicos, linkedMedicamentos, tratamento?.medico_ids]);

  const linkedHospitais = useMemo(() => {
    const ids = new Set(tratamento?.hospital_ids || []);

    return hospitais.filter((hospital: Hospital) =>
      Boolean(hospital.id && ids.has(hospital.id)),
    );
  }, [hospitais, tratamento?.hospital_ids]);

  const linkedLocais = useMemo(() => {
    const ids = new Set(tratamento?.local_ids || []);

    return locais.filter((local: LocalSaude) =>
      Boolean(local.id && ids.has(local.id)),
    );
  ...
  const medicamentosAtivos = useMemo(
    () =>
      medicamentosComAlertas.filter(
        (medicamento) => medicamento.status !== "descontinuado",
      ),
    [medicamentosComAlertas],
  );

  const medicamentosDescontinuados = useMemo(
    () =>
      medicamentosComAlertas.filter(
        (medicamento) => medicamento.status === "descontinuado",
      ),
  ...
  const medicamentosPrecisandoAtencao = useMemo(
    () =>
      medicamentosAtivos.filter(
        (medicamento) =>
          medicamento.receitaVencida || medicamento.insight?.deveRenovar,
      ),
    [medicamentosAtivos],
  );
  ...
  const custoHistorico = useMemo(
    () =>
      linkedRenovacoes.reduce(
        (total, renovacao) => total + getPrecoSeguro(renovacao.preco),
        0,
      ),
    [linkedRenovacoes],
  );

  const aquisicoesComPreco = useMemo(
    () =>
      linkedRenovacoes.filter(
        (renovacao) => getPrecoSeguro(renovacao.preco) > 0,
      ).length,
    [linkedRenovacoes],
  );
  ...
  const economiasPorMedicamento = useMemo<EconomiaMedicamento[]>(() => {
    const result: EconomiaMedicamento[] = [];
    ...
    for (const medicamento of linkedMedicamentos) {
      ...
      const historico = linkedRenovacoes.filter(
        (renovacao) => renovacao.medicamento_id === medicamento.id,
      );

      const economia = calcularEconomia(historico);

      if (
        !economia ||
        !Number.isFinite(economia.percentual) ||
        !Number.isFinite(economia.economia)
      ) {
        continue;
      }
  ...
  const statsSus = useMemo(() => {
    const total = linkedMedicamentos.length;

    const totalSus = linkedMedicamentos.filter(
      (medicamento) => medicamento.tipo_aquisicao === "sus",
    ).length;
    ...
    return {
      ...
      isAllSus: total > 0 && totalSus === total,
    };
  }, [linkedMedicamentos]);
  ...
  const panorama = useMemo(() => {
    const primeiraAquisicao =
      linkedRenovacoes.length > 0
        ? linkedRenovacoes[linkedRenovacoes.length - 1]?.data_aquisicao ||
          linkedRenovacoes[linkedRenovacoes.length - 1]?.data
        : undefined;

    const ultimaAquisicao =
      linkedRenovacoes[0]?.data_aquisicao || linkedRenovacoes[0]?.data;

    return {
      renovacoes: linkedRenovacoes.length,
      ...
      medicamentosDescontinuados: medicamentosDescontinuados.length,

      medicamentosPrecisandoAtencao: medicamentosPrecisandoAtencao.length,
    };
  }, [
    linkedRenovacoes,
  ...
  //
  // A página resolve o grafo relacional. O cérebro recebe
  // somente registros da pessoa ativa e já limitados a este
  // tratamento, mantendo sua independência de React e Dexie.
  // ==========================================================

  const timeline = useMemo<HealthTimelineEvent[]>(() => {
    if (
      !activePersonId ||
      !tratamento ||
      !belongsToPerson(tratamento, activePersonId)
    ) {
      return [];
    }

    return gerarLinhaDoTempoSaude(
      {
        personId: activePersonId,

        medicamentos: linkedMedicamentos.filter((medicamento) =>
          belongsToPerson(medicamento, activePersonId),
        ),

        doseLogs: [],

        renovacoes: linkedRenovacoes.filter((renovacao) =>
          belongsToPerson(renovacao, activePersonId),
        ),

        tratamentos: [tratamento],

        registrosSaude: linkedRegistros.filter((registro) =>
          belongsToPerson(registro, activePersonId),
        ),

        consultas: linkedConsultas.filter((consulta) =>
          belongsToPerson(consulta, activePersonId),
        ),

        exames: linkedExames.filter((exame) =>
          belongsToPerson(exame, activePersonId),
        ),

        cirurgias: linkedCirurgias.filter((cirurgia) =>
          belongsToPerson(cirurgia, activePersonId),
        ),

        cids: cidsVinculados.filter((cid) =>
          belongsToPerson(cid, activePersonId),
        ),
  ...
  if (!tratamento) {
    return (
      <main className="flex min-h-[100dvh] flex-col items-center justify-center bg-void px-6 text-center">
        <FolderHeart size={36} className="text-ink-muted" />
        ...
        <button
          type="button"
          onClick={() => router.replace("/saude/tratamentos")}
  ...
  const menuOptions = [
...

    {
      ...
      path: `/saude/documentos/novo?tratamento_id=${id}`,
    },
      ...
      <main className="relative min-h-screen bg-void pb-[calc(8rem+env(safe-area-inset-bottom))]">
              ...
              <div className="relative">
                <button
                  ...
                  onClick={(event) => {
                    ...
                    setIsMenuFlutuanteOpen((current) => !current);
                  }}
        ...
        <section className="space-y-6 px-5 pt-6">
              ...
              <div className="min-w-0 pt-1">
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  >
                    ...
                    {getStatusLabel(tratamento.status)}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="relative z-10 mt-4 rounded-xl border border-surface-border/40 bg-surface-raised/50 p-3">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {cidsInsights.map((cid) => {
                    const cidTheme = getClinicalTheme(
                ...
                {cidsInsights.some((cid) =>
                  Boolean(cid.insight?.alertaClinico),
                ) && (
                  <div className="mt-3 flex items-start gap-2 rounded-xl border border-amber-400/20 bg-amber-400/5 p-2.5">
                    ...
                    <p className="text-[11px] leading-relaxed text-ink-muted">
                      ...
                      {cidsInsights
                        .map((cid) => cid.insight?.alertaClinico)
                        .filter(Boolean)
                        .join(" • ")}
                    </p>
                  </div>
          ...
          >
            ...
            {timeline.length === 0 ? (
              <div className="rounded-[24px] border border-surface-border/50 bg-surface-raised/50 p-5 text-center">
                <p className="text-sm text-ink-muted">
                  Nenhum evento clínico vinculado a este tratamento.
                </p>
              </div>
            ) : (
              <div className="ml-3 space-y-2 border-l-2 border-surface-border/50 pl-4">
                {timeline.map((event) => {
                  const href = getTimelineHref(event);
                  ...
                  return (
                    <button
                      ...
                      className="relative flex w-full items-center gap-3 rounded-2xl border border-surface-border/50 bg-surface p-3 text-left transition-all enabled:active:scale-[0.98] disabled:cursor-default"
                    >
                      <span className="absolute -left-[23px] top-5 h-2.5 w-2.5 rounded-full border-2 border-void bg-ice ring-1 ring-surface-border/50" />

                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-ice/10 text-ice">
                        <TimelineEventIcon tipo={event.tipo} />
                      </span>
          ...
          >
              ...
              <div className="rounded-[20px] border border-surface-border/50 bg-surface p-3.5">
                ...
                <p className="mt-0.5 text-[10px] text-ink-muted">
                  {panorama.ultimaAquisicao
                    ? `Última em ${formatDateDisplay(panorama.ultimaAquisicao)}`
            ...
            {panorama.primeiraAquisicao &&
              panorama.ultimaAquisicao &&
              panorama.primeiraAquisicao !== panorama.ultimaAquisicao && (
                <p className="px-1 text-[10px] leading-relaxed text-ink-muted">
                  ...
                  <span className="font-medium text-ink-primary">
                    {formatDateDisplay(panorama.primeiraAquisicao)}
                  </span>{" "}
                  até{" "}
                  <span className="font-medium text-ink-primary">
                    {formatDateDisplay(panorama.ultimaAquisicao)}
                  </span>
                  .
                </p>
              )}
              ...
              <div className="flex items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 font-display text-xl font-semibold text-ink-primary">
                    {aquisicoesComPreco > 0
                      ? formatCurrency(custoHistorico)
                      : "Sem valores"}
                  </p>
                </div>
                ...
                <div className="text-right">
                  <p className="text-xs font-medium text-ink-primary">
                    {aquisicoesComPreco} compra
                    {aquisicoesComPreco !== 1 ? "s" : ""} com preço
                  </p>
          ...
          {economiasPorMedicamento.length > 0 && !dismissEconomia && (
            <motion.div
              initial={{
                opacity: 0,
                y: 10,
            ...
            >
              ...
              <div className="space-y-2 pt-1">
                {economiasPorMedicamento.slice(0, 3).map((item) => {
                  const melhorou = item.economia.economia > 0;
                        ...
                        <p className="mt-0.5 text-[10px] leading-relaxed text-ink-muted">
                          {melhorou
                            ? `Última aquisição ficou ${formatCurrency(
                                Math.abs(item.economia.economia),
                              )} abaixo da média anterior (${Math.abs(
                                item.economia.percentual,
                              ).toFixed(1)}%).`
                            : `Última aquisição ficou ${formatCurrency(
                                Math.abs(item.economia.economia),
          ...
          <motion.div
            ...
            className="space-y-3"
          >
            <SectionTitle icon={<Users size={15} />} title="Rede de Apoio" />

            {linkedMedicos.length === 0 &&
            linkedHospitais.length === 0 &&
            linkedLocais.length === 0 ? (
          ...
          {linkedRenovacoes.length > 0 && (
            ...
            >
              <SectionTitle
                ...
                action={
                  linkedRenovacoes.length > 5 ? (
                    <span className="text-[10px] font-medium text-ink-faint">
                      {linkedRenovacoes.length} registros
                    </span>
                  ) : undefined
                }
              />
              ...
              <div className="space-y-2">
                {linkedRenovacoes.slice(0, 5).map((renovacao) => {
                  const medicamento = linkedMedicamentos.find(
                    ...
                    >
                      <DetailInfoRow
                        ...
                        label={formatDateDisplay(
                          renovacao.data_aquisicao || renovacao.data,
                        )}
                        action={
                      ...
                      >
                        <div className="flex min-w-0 items-center justify-between gap-3">
                          ...
                          {getPrecoSeguro(renovacao.preco) > 0 && (
                            <span className="shrink-0 text-sm font-semibold text-emerald-400">
                              {formatCurrency(renovacao.preco as number)}
                            </span>
                          )}
                        </div>
                      </DetailInfoRow>
              ...
              {linkedRenovacoes.length > 5 && (
                <p className="pt-1 text-center text-[10px] text-ink-muted">
                  E mais {linkedRenovacoes.length - 5} aquisição(ões).
                </p>
              )}
            </motion.div>
          )}
            ...
            <SectionTitle
              ...
              action={
                medicamentosAtivos.length > 0 ? (
                  <span className="text-[10px] font-medium text-ink-faint">
                    {medicamentosAtivos.length} ativo
                    {medicamentosAtivos.length !== 1 ? "s" : ""}
                  </span>
                ) : undefined
              }
            />
                      ...
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex min-w-0 items-center gap-2">
                            ...
                            {!medicamento.receitaVencida &&
                              medicamento.insight?.deveRenovar && (
              ...
              <SectionTitle
                ...
                action={
                  <span className="text-[10px] font-medium text-coral">
                    {medicamentosDescontinuados.length} descontinuado
                    {medicamentosDescontinuados.length !== 1 ? "s" : ""}
                  </span>
                }
              />

              <div className="ml-3 space-y-3 border-l-2 border-surface-border/50 pl-4">
                {medicamentosDescontinuados.map((medicamento) => (
                  <button
                    key={medicamento.id}
                    type="button"
                    onClick={() => {
            ...
            <SectionTitle
              ...
              action={
                linkedDocuments.length > 0 ? (
                  <span className="text-[10px] font-medium text-ink-faint">
                    {linkedDocuments.length} documento
                    {linkedDocuments.length !== 1 ? "s" : ""}
                  </span>
                ) : undefined
              }
            />
            ...
            {linkedDocuments.length === 0 ? (
              ...
              >
                {linkedDocuments.map((document) => (
                  <motion.div key={document.id} variants={cardVariants}>
                    <DocumentCard
                      document={document}
                      href={
                        document.id
