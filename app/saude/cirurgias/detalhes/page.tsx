      ...
      <main className="min-h-screen bg-void pb-[calc(8rem+env(safe-area-inset-bottom))]">
        ...
        <section className="px-5 pt-6 space-y-5">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-surface-border/40 relative z-10">
              ...
              >
                <p className="text-sm font-medium text-ink-primary truncate">
                  {medico ? `Dr(a). ${medico.nome}` : "Não vinculado"}
                </p>
              </DetailInfoRow>
            ...
            {cirurgia.observacoes && (
              <div className="rounded-2xl bg-surface-raised/60 p-4 border border-surface-border/40 relative z-10">
                ...
                <p className="text-sm text-ink-primary whitespace-pre-wrap mt-1.5 leading-relaxed">
                  {cirurgia.observacoes}
                </p>
              </div>
            )}
          ...
          <motion.div
            ...
            className="space-y-4"
          >
            <SectionTitle icon={<FileText size={15} />} title="Registros Clínicos do Procedimento" />
            ...
            <div className="grid grid-cols-1 gap-3">
              <section className="rounded-[24px] border border-surface-border/50 bg-surface p-4 shadow-sm space-y-3">
                ...
                {medicamentosPosOperatorios.length === 0 ? (
                  <p className="text-xs text-ink-muted py-2">
                    Nenhum medicamento vinculado a esta data ou equipe médica.
                  </p>
                ) : (
                  <div className="space-y-2">
                    {medicamentosPosOperatorios.map((m: Medicamento) => (
                      <div
                        ...
                        onClick={() => {
                          trigger("vibrate");
                          router.push(`/saude/medicamentos/detalhes?id=${m.id}`);
                        }}
                      ...
                      >
                        <div>
                          <p className="text-sm font-medium text-ink-primary">
                            {m.nome} · <span className="text-coral">{m.dosagem}</span>
                          </p>
                          <p className="text-[11px] text-ink-muted">
                            Prescrito em: {formatDateDisplay(m.data_receita)}
                          </p>
                        </div>
              ...
              <section className="rounded-[24px] border border-surface-border/50 bg-surface p-4 shadow-sm space-y-3">
                ...
                {examesPreOperatorios.length === 0 ? (
                  <p className="text-xs text-ink-muted py-2">
                    Nenhum exame pré-operatório registrado com esta equipe.
                  </p>
                ) : (
                  <div className="space-y-2">
                    {examesPreOperatorios.map((e: Exame) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-[11px] text-ink-muted">
                            Solicitado em: {formatDateDisplay(e.data)}
                          </p>
                        </div>
                      </div>
                    ))}
          ...
          {cirurgia.status === "agendada" && (
            ...
            >
              <SectionTitle icon={<CheckCircle2 size={15} />} title="Ações de Atualização" />
