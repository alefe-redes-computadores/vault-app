...
function DetalhesRegistroSaudeContent() {
  ...
  //
  // A regra temporal e a comparação de intensidade pertencem
  // ao health-insights.ts. A página apenas apresenta fatos do
  // histórico person-scoped preparado pelo repository.
  // ==========================================================
  ...
  const panoramaHistorico =
    useMemo(
      () => {
        ...
        return analisarSerieRegistrosSaude(
          registro,
          historicoSimilar
        );
      },
  ...
  const ocorrenciasAnteriores =
    useMemo(
      () => {
        if (!panoramaHistorico) {
          return [];
        }

        const ids = new Set(
          panoramaHistorico.idsAnteriores
        );

        return historicoSimilar.filter(
          (item) =>
            Boolean(
