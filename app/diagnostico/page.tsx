...
export default function DiagnosticoPage(){
 ...
 const failed=queue.filter(item=>item.failed||Number(item.retry_count||0)>=5).length;const deferred=queue.filter(item=>!item.failed&&Number(item.retry_count||0)<5&&item.next_retry_at&&new Date(item.next_retry_at).getTime()>Date.now()).length;
