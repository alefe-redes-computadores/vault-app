...
// Compatibilidade V18: a regra central equivale a Number(item.retry_count||0)>=5.
...
export default function DiagnosticoPage(){
 ...
 const queueHealth=useMemo(()=>summarizeSyncQueue(queue),[queue]);const failed=queueHealth.failed;const deferred=queueHealth.deferred;
 const byTable=useMemo(()=>queue.reduce<Record<string,{total:number;failed:number}>>((result,item)=>{const current=result[item.table]||{total:0,failed:0};current.total+=1;if(getSyncQueueState(item)==="failed")current.failed+=1;result[item.table]=current;return result},{}),[queue]);
 ...
 return <PageTransition><main className="min-h-screen bg-void px-5 pb-28 text-ink-primary">
  ...
  <section className="mt-5 grid grid-cols-3 gap-2">{([["Prontos",queueHealth.ready],["Com falha",failed],["Aguardando",deferred]] as const).map(([label,value])=><div key={label} className={`rounded-2xl border bg-surface p-3 text-center ${label==="Com falha"&&value>0?"border-coral/35":"border-surface-border"}`}><strong className="text-xl">{value}</strong><p className="text-[11px] text-ink-muted">{label}</p></div>)}</section>
