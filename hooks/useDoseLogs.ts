...
export function useDoseLogs(dataEspecifia?: string) {
  ...
  const doseLogs = useLiveQuery(
    () => {
      ...
      if (dataEspecifia) query = query.and((log) => log.data === dataEspecifia);
  ...
  const marcarComoTomada = useCallback(async (medicamentoId: string, dataDose: string, horario: string) => {
    if (!user) throw new Error('Usuário não autenticado');
    return await safeSetDoseLog({
      user_id: user.id,
  ...
  const marcarComoIgnorada = useCallback(async (medicamentoId: string, dataDose: string, horario: string) => {
    if (!user) throw new Error('Usuário não autenticado');
    return await safeSetDoseLog({
      user_id: user.id,
      person_id: activePersonId || undefined,
  ...
  const desmarcarDose = useCallback(async (medicamentoId: string, dataDose: string, horario: string) => {
    if (!user) throw new Error('Usuário não autenticado');
    return await safeSetDoseLog({
      user_id: user.id,
      person_id: activePersonId || undefined,
